...
public class ViewMongoTemplate implements ViewMongoOperations {
    ...
    public boolean hasUpdated(String id, Long version, String viewName) {
        return this.mongoOperations.count(query(where("id").is(id).and(VERSION_KEY).gt(version)), View.class, viewName) > 0;
    }
    ...
    public <T extends View> void update(Query query, Update update, Class<T> viewClass) {
        this.mongoOperations.findAndModify(query, update, viewClass);
    }
